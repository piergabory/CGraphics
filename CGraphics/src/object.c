...
Object importOBJ(char* filepath, ShaderProgram program) {
    ...
    for (size_t face = 0; face < attributes.num_face_num_verts; face++) {
        ...
        for (size_t face_vertex = 0; face_vertex < attributes.face_num_verts[face]; face_vertex++) {
            tinyobj_vertex_index_t idx = attributes.faces[face_offset + face_vertex];
            float *attribute_byte_position = vertice_attributes_buffer + (3 * face + face_vertex) * stride;
